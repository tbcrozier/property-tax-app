#!/usr/bin/env python3
"""
Extract parcel data from St. Louis County.
Data source: St. Louis County ArcGIS MapServer (AGS_Parcels)
"""
...
# St. Louis County ArcGIS MapServer endpoint for Parcels
PARCELS_URL = "https://maps.stlouisco.com/hosting/rest/services/Maps/AGS_Parcels/MapServer/0/query"
LAYER_INFO_URL = "https://maps.stlouisco.com/hosting/rest/services/Maps/AGS_Parcels/MapServer/0"

# BigQuery destination
DEFAULT_BQ_TABLE = "public-data-dev.st_louis.parcels"
...
def extract_parcels(total_records=100, batch_size=1000):
    ...
    Args:
        ...
        batch_size: Records per API request (max 6000 for this API)
    ...
    print(f"Fetching {total_records} parcels from St. Louis County...")
...
def clean_for_bigquery(df):
    ...
    - Remove columns with invalid BigQuery names
    - Convert timestamps to ISO format
    - Handle null values
    """
    # Drop columns with invalid BigQuery names (contain parentheses, dots, etc.)
    invalid_cols = [col for col in df.columns if "(" in col or ")" in col or col.startswith("SHAPE")]
    if invalid_cols:
        print(f"Dropping columns with invalid names: {invalid_cols}")
...
def main():
    parser = argparse.ArgumentParser(description="Extract St. Louis County parcel data")
    parser.add_argument(
        "--count",
        type=int,
        default=100,
    ...
    assessment_cols = ["LOCATOR", "PROP_ADD", "APPLANDVAL", "APPIMPVAL", "TOTAPVAL", "MUNICIPALITY"]
